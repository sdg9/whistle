...
function base64MessagePackToDecode(base64) {
  if (base64) {
...
var Textarea = React.createClass({
  ...
  submit: function(e) {
    ...
    if (this.props.base64) {
      // const buffer = Buffer.from(this.props.base64, 'base64');
      // var decodedFromBuffer = msgpack.decode(buffer);
      // value = decodedFromBuffer;
      value = base64MessagePackToDecode(this.props.base65);
    }
  ...
  render: function() {
    ...
    //   const buffer = Buffer.from(this.props.base64, 'base64');
    //   var decodedFromBuffer = msgpack.decode(buffer);
    //   console.log('Base64 decode: ', decodedFromBuffer);
    // }
    var value = this.props.value || '';
    if (this.props.base64) {
      const buffer = Buffer.from(this.props.base64, 'base64');
      var decodedFromBuffer = msgpack.decode(buffer);
      value = JSON.stringify(decodedFromBuffer);
    }
      ...
      >
        ...
        <div className={'w-textarea-bar' + (value ? '' : ' hide')}>
          ...
          {isHexView ? (
            <CopyBtn
              name={isHexView ? 'Hex' : ''}
              value={util.getHexText(this.props.value)}
            />
          ) : (
            undefined
          )}
